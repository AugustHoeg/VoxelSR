...
def main(opt: DictConfig):
    ...
    # Load datasets of interest for cross dataset evaluation
    datasets = []
    synthetic = False
    if 'datasets' in opt['dataset_opt']:
        datasets = opt['dataset_opt']['datasets']
        print("Datasets for cross dataset evaluation:", datasets)
    if 'synthetic' in opt['dataset_opt']:
        synthetic = opt['dataset_opt']['synthetic']
    ...
    if len(datasets) > 0:
        opt['dataset_opt']['datasets'] = datasets
        opt['dataset_opt']['synthetic'] = synthetic
    else:
        print("No datasets specified for cross dataset evaluation, using dataset from options file.")
