    ...
    def forward(self, x, mask=None):
        ...
        if self.rpb:
            relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
                ...
                self.window_size[0] * self.window_size[1] * self.window_size[2], -1)  # Wh*Ww*Wd, Wh*Wd*Ww, nH
...
class PatchUnEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        img_size = to_3tuple(img_size)
        patch_size = to_3tuple(patch_size)
        patches_resolution = patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1],
...
class SuperFormer(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 embed_dim=48, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, rpb=True, patch_norm=True,
                 use_checkpoint=True, upscale=2, img_range=1., upsampler='', resi_connection='1conv',
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if self.patch_size > 1:
                self.upsample_feat = nn.Sequential(
                    nn.ConvTranspose3d(embed_dim, embed_dim, kernel_size=self.patch_size, stride=self.patch_size),
            ...
            if self.output_type == 'residual':
                ...
                self.conv_before_last = nn.Sequential(nn.Conv3d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
                #self.conv_last = nn.Conv3d(num_feat, num_out_ch, 1, 1, 0)

        # Adding upsampling modules // August
        from models.models_3D import SRBlock3D
        recon_feats = num_feat
        ...
        if upscale == 2:
            ...
            self.SR0 = SRBlock3D(num_feat, num_feat // 2, k_size=6, pad=2, upsample_method=upsampler, upscale_factor=3, use_checkpoint=False)
            recon_feats = num_feat // 2
        elif upscale == 4:
            self.SR0 = SRBlock3D(num_feat, num_feat // 2, k_size=6, pad=2, upsample_method=upsampler, upscale_factor=2, use_checkpoint=False)
        ...
        self.conv_last = nn.Conv3d(recon_feats, num_out_ch, 1, 1, 0)
    ...
    def forward_features(self, x, x_feat):
        ...
        for layer in self.layers:
            #x_feat = checkpoint.checkpoint(layer, x_feat, x_size)
            #x_vol = checkpoint.checkpoint(layer, x_vol, x_size)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if self.patch_size > 1:
                ...
                x_feat, x_vol = self.forward_features(x, x_first)
                res = (self.conv_after_body(x_feat) + self.conv_after_body(x_vol)) / 2
                res = self.upsample_feat(res)
                res = res + x_first
            else:
            ...
            if self.output_type == 'residual':
                ...
                # Inserting upsampling module before last convolution
                '''
                As standard, Superformer has a convolutional module at the end of feature extraction which consists of 
                2 layers. One (conv_before_last) reduces channels from emb_dim (252) to num_feat (126), and the other one 
                (conv_last) which reduces from num_feat to 1 (number of output channels). 
                To add the upsampling module, we insert the upsampling blocks after conv_before_last to ease the computation.
                The upsampled features are then parsed through the conv_last layer.
                // August
                '''
                if self.upscale == 1:
                    res = self.conv_last(self.conv_before_last(res))
                else:
                    res = self.conv_before_last(res)
                    if self.upscale == 2:
                        res = self.SR0(res)
                    elif self.upscale == 3:
                        res = self.SR0(res)
        ...
        # Disable redundancy here // August
        #x = x / self.img_range + self.mean

        #return x[:, :, :H * self.upscale, :W * self.upscale, :D * self.upscale]  # Original
        #return res[:, :, :H * self.upscale, :W * self.upscale, :D * self.upscale]  # Replace x with res // August
        return res  # Just return res // August
...
def test():
    ...
    print("Test SuperFormer")
    generator = SuperFormer(img_size=patch_size, patch_size=2, in_chans=1,
                 embed_dim=252, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=8, mlp_ratio=2, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, rpb=True, patch_norm=True,
                 use_checkpoint=True, upscale=up_factor, img_range=1., upsampler="pixelshuffle3D", resi_connection='1conv',  # pixelshuffle3D Monaipixelshuffle trilinear
    ...
    # Backward pass to estimate memory consumption of backpropagating the model
    criterion = nn.MSELoss()
    loss = criterion(x_hr, gen_out)
    loss.backward()
