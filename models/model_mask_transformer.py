...
class ModelMaskTransformer(ModelBase):
    """Trains a bidirectional transformer (MaskGIT-style) over VQ codebook indices.

    The VQ model is frozen. During training a random fraction of tokens are replaced
    with a dedicated mask token (index = num_embeddings - 1) using an arccos schedule,
    and the transformer learns to predict the original tokens at those positions via
    (non-causal) attention.

    During inference all tokens begin masked and are revealed iteratively: each pass
    predicts all masked positions and keeps the top-t most confident predictions,
    following an arccos unmasking schedule over n_steps refinement rounds.

    Config requirements:
      netG.num_embeddings      = vq_codebook_size  (mask token reuses the +1 tok_emb slot)
      netG.is_causal           = False                 (bidirectional attention)
      model_opt.mask_schedule  (optional): "arccos" | "cosine" | "linear" | "square"
    """
    ...
    def load_vq_model(self):
        assert 'pretrained_vqmodel_id' in self.opt['path'], \
            "Must specify pretrained_vqmodel_id in path for ModelMaskTransformer."
        ...
        assert self.num_embeddings == self.vq_model.codebook.num_embeddings, (
            ...
            f"VQ model has {self.vq_model.codebook.num_embeddings}."
        )
    ...
    def encode_to_indices(self, x: torch.Tensor):
        ...
        Args:
            x: (B, C, D, H, W)
        Returns:
            q_indices:    (B, L) int64  where L = D'*H'*W'
            latent_shape: (D', H', W')
        """
    ...
    def sample_from_transformer(self, n_samples: int = 1, n_steps: int = 12,
        ...
        """
        assert self.latent_shape is not None, "latent_shape not set; call encode_to_indices first."
        L = int(np.prod(self.latent_shape))
    ...
    def optimize_parameters_amp(self, current_step, update=False):
        ...
        with torch.amp.autocast("cuda", dtype=self.mixed_precision):
            logits = self.netG(masked_indices)    # (B, L, num_embeddings)
    ...
    def optimize_parameters(self, current_step, update=False):
        ...
        logits = self.netG(masked_indices)
        self.gen_loss = (
            F.cross_entropy(logits[mask], q_indices[mask])
            / self.num_accum_steps_G
        )
    ...
    def current_visuals(self):
        ...
        out_dict['E_maskgit'] = rgb2gray(E_maskgit.detach()[0].float().cpu(), channel_dim=0)
