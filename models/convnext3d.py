    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for x, masks in zip(x_list, masks_list):
            ...
            for i in range(4):
                ...
                x = self.stages[i](x)
            x_pool = x.mean([-3, -2, -1])  # global average pooling, (N, C, H, W) -> (N, C)
...
if __name__ == "__main__":
    ...
    model = ConvNeXt(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], in_chans=in_chans, drop_path_rate=0.1)

    model.eval()  # inference mode

    # Create a random input: batch size 1, 3 channels, 224x224 image
    x = torch.randn(1, in_chans, 224, 224, 224)
