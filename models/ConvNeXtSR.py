...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, dp_rates, layer_scale_init_value):
        ...
        # self.conv = nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def __init__(
        ...
        **ignored_kwargs,
        ...
        for i in range(len(depths)):
            ...
            stage = BasicLayer(dims[i], depths[i], dp, layer_scale_init_value)
        ...
        # Upsampling
        if self.up_factor == 2:
            self.SR0 = SRBlock3D(dims[-1], dims[-1], k_size=6, pad=2, upsample_method=upsample_method, upscale_factor=2)
        elif self.up_factor == 3:
            self.SR0 = SRBlock3D(dims[-1], dims[-1], k_size=6, pad=2, upsample_method=upsample_method, upscale_factor=3)
        elif self.up_factor == 4:
            self.SR0 = SRBlock3D(dims[-1], dims[-1], k_size=6, pad=2, upsample_method=upsample_method, upscale_factor=2)
            self.SR1 = SRBlock3D(dims[-1], dims[-1], k_size=6, pad=2, upsample_method=upsample_method, upscale_factor=2)

        # reconstruction
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        self.HRconv = nn.Conv3d(dims[-1], dims[-1], 3, 1, 1, bias=True)

        self.conv_last = nn.Sequential(
            nn.Conv3d(in_channels=dims[-1], out_channels=in_chans, kernel_size=3, stride=1, padding=1, bias=True)
        )
