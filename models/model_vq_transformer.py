...
class ModelTransformerVQ(ModelBase):
    ...
    def optimize_parameters_amp(self, current_step, update=False):
        ...
        sos = q_indices.new_full((q_indices.shape[0], 1), self.num_embeddings)
        idx_in = torch.cat([sos, q_indices[:, :-1]], dim=1)

        with torch.amp.autocast("cuda", dtype=self.mixed_precision):
            logits = self.netG(idx_in)  # (B, L, num_embeddings)
            self.gen_loss = (
