"""
2D VQVAE Encoder / Decoder for the VARSR baseline.

Faithful copy of the official VARSR ``models/basic_vae.py``
(https://github.com/quyp2000/VARSR/blob/master/models/basic_vae.py), which in
turn follows Stable-Diffusion's first-stage autoencoder
(https://github.com/CompVis/stable-diffusion/blob/main/ldm/modules/diffusionmodules/model.py).

Kept verbatim except that ``in_channels`` is already a constructor argument, so
the single-channel (grayscale) VoxelSR use-case needs no code change — just pass
``in_channels=1``. The module names and structure are preserved exactly so that
official RGB checkpoints (``in_channels=3``) load with strict key matching.
"""
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, dropout):  # conv_shortcut: always False in VAE
        super().__init__()
