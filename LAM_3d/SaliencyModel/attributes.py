...
def attr_grad(tensor, h, w, d, window=8, reduce='sum'):
    """
    :param tensor: B, C, H, W, D tensor
    :param h: h position
    :param w: w position
    :param d: d position
    :param window: size of window
    ...
    d_grad = torch.pow(tensor[:, :, :, :, :d_x - 1] - tensor[:, :, :, :, 1:], 2)
    grad = torch.pow(h_grad[:, :, :, :-1, :-1] + w_grad[:, :, :-1, :, :-1] + d_grad[:, :, :-1, :-1, :], 1 / 2)
    crop = grad[:, :, h: h + window, w: w + window, d: d + window]
    return reduce_func(reduce)(crop)

def attr_grad_2d(tensor, h, w, window=8, reduce='sum'):
    """
    :param tensor: B, C, H, W tensor
    :param h: h position
    :param w: w position
