...
def vis_saliency_new(map, zoomin=4):
    """
...
def prepare_3d_images(hr_path, scale=4):
    hr_np = np.load(hr_path)
    # sizex, sizey = hr_np.size
    #hr_pil = hr_pil.crop((0, 0, sizex - sizex % scale, sizey - sizey % scale))
    #sizex, sizey = hr_pil.size
    lr_np = ndimage.gaussian_filter(hr_np, sigma=1)
    lr_np = block_reduce(lr_np, block_size=(1,scale, scale, scale), func=np.mean)
    #lr_pil = hr_pil.resize((sizex // scale, sizey // scale), Image.BICUBIC)
    return lr_np, hr_np
...
def isotropic_gaussian_kernel(l, sigma, epsilon=1e-5):
    ...
    kernel = np.exp(-(xx ** 2 + yy ** 2 + zz ** 2) / (2. * (sigma + epsilon) ** 2))
    return kernel / np.sum(kernel)

def isotropic_gaussian_kernel_2d(l, sigma, epsilon=1e-5):
    ax = np.arange(-l // 2 + 1., l // 2 + 1.)
