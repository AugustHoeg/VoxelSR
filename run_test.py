...
def main(opt: DictConfig):
    ...
    for sample_idx, baseline_batch in enumerate(baseline_loader):
        ...
        if True:
            ...
            write_tiff(img_E.squeeze().cpu().numpy(), os.path.join(image_dir, "full_SR_sample_%d.tiff" % sample_idx))
            write_tiff(img_H.squeeze().cpu().numpy(), os.path.join(image_dir, "full_HR_sample_%d.tiff" % sample_idx))
            write_tiff(img_L.squeeze().cpu().numpy(), os.path.join(image_dir, "full_LR_sample_%d.tiff" % sample_idx))
