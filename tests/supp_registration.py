...
if __name__ == '__main__':
    ...
    img_idx = 10 # 0 to 4
    base_dir = "../../downloaded_data/VoDaSuRe/supplementary/slices/"

    #sample_dirs = [os.path.join(base_dir, name) for name in sample_names]

    HR_paths = glob.glob(os.path.join(base_dir, "*HR.png"))
    LR_paths = glob.glob(os.path.join(base_dir, "*LR.png"))
    LR_up_paths = glob.glob(os.path.join(base_dir, "*LR_up.png"))
    ...
    HR = np.array(Image.open(HR_paths[img_idx]))
    ...
    for j in range(col):
        ...
        name = os.path.basename(HR_paths[img_idx]).replace("_HR.png", "").replace("_", " ")
        ...
        if j == 0:  # show full slice
            ...
            img = HR
            img, _ = crop_image_at_location(img.reshape([*img.shape, 1]), red_box_size, red_box_coords)
            ax.imshow(img, cmap='gray', vmin=0, vmax=65535)
            ...
            img, _ = crop_image_at_location(img.reshape([*img.shape, 1]), red_box_size, red_box_coords)
            ax.imshow(img, cmap='gray', vmin=0, vmax=65535)
            ax.text(0.5, -0.02, r"REG crop $(4\times)$", ha='center', va='top', fontsize=18, transform=ax.transAxes)

        elif j == 3:  # show checkerboard image

            HR_crop, _ = crop_image_at_location(HR.reshape([*HR.shape, 1]), red_box_size, red_box_coords)
            LR_crop, _ = crop_image_at_location(LR_up.reshape([*LR_up.shape, 1]), red_box_size, red_box_coords)

            HR_crop = HR_crop.astype(np.float32)
            LR_crop = LR_crop.astype(np.float32)

            checker = compare_images(HR_crop[:, :, 0], LR_crop[:, :, 0], method='checkerboard', n_tiles=(6, 6))

            ax.imshow(checker, cmap='gray')
            ax.text(0.5, -0.02, "Checkerboard image", ha='center', va='top', fontsize=18, transform=ax.transAxes)

        else:  # show difference image
            ...
            ax.imshow(np.abs(LR_crop - HR_crop), cmap='gray', vmin=0, vmax=65535)
