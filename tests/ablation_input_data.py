...
if __name__ == '__main__':
    ...
    large_image_string = r"Ablation_input" # r"Registered"
    ...
    for i in range(row):
        for j in range(col):
            ...
            if j == 0:
                ...
                plot_prediction = True
                use_registered = False  # Change to False for downsampled data
                upper_title = "Train: Down. / Test: Down."
                ...
                plot_prediction = True
                use_registered = True  # Change to False for downsampled data
                upper_title = "Train: Reg. / Test: Reg."
                ...
                plot_prediction = True
                use_registered = True  # Change to False for downsampled data
                upper_title = "Train: Down. / Test: Reg."
            ...
            ax.set_title(f"{upper_title}", fontsize=15, pad=0., y=1.03)
            if j == 0:
                ax.text(0.5, -0.02, f"TV: {tv:.2f}", ha='center', va='top', fontsize=15, transform=ax.transAxes)
            else:
                ax.text(0.5, -0.02, f"{box_name}, TV: {tv:.2f}", ha='center', va='top', fontsize=15, transform=ax.transAxes)

            ax.set_xticks([])
            ax.set_yticks([])

    #plt.subplots_adjust(hspace=0.45)
    save_path = f"../figures/{large_image_string}_ablation_input_{img_idx}_{red_box_size}.pdf"
