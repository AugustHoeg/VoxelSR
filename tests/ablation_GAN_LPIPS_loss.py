...
if __name__ == '__main__':
    ...
    fig = plt.figure(figsize=(7*1.5, 2.6*1.5 if plot_metrics else 2.1*1.5), constrained_layout=True)
    ...
    for i in range(row):
        for j in range(col):
            ...
            if j == 0:
                ...
                LR_title = r"HR data"  # "Downsampled"

                large_img_size = 512
                red_box_coords = (150, 150)
                red_box_size = 256
                lpips_str = f"Reference image"
            ...
            if j == 3:
                ...
                red_box_coords = (150, 150)
                red_box_size = 256
                lpips_str = f"w. LPIPS + GAN"


            model_dirs = [
                f"{base_dir}/{model_names[0]}/{dataset}/{group_dir}/"
            ]
            ...
            vmax, vmin = 1.0, -1.0
            E_tensor = E_img.unsqueeze(0).float() / 255.0
            H_tensor = H_img.unsqueeze(0).float() / 255.0
            E_tensor = ((E_tensor - E_tensor.min()) / (E_tensor.max() - E_tensor.min())) * (vmax - vmin) + vmin
            H_tensor = ((H_tensor - H_tensor.min()) / (H_tensor.max() - H_tensor.min())) * (vmax - vmin) + vmin
    ...
    save_path = f"../figures/{large_image_string}_GAN_LPIPS_{img_idx}_{red_box_size}.pdf"
