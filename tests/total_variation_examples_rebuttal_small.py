...
if __name__ == '__main__':
    ...
    img_idx_list = [1, 10, 19, 28]  # For VoDaSuRe_DOWN
    ...
    fig = plt.figure(figsize=(14.5, 3 if plot_metrics else 4), constrained_layout=True)
    ...
    for i in range(row):
        ...
        for j in range(col):
            ...
            if lpips_value is not None:
                ax.text(0.5, -0.03, f"TV: {tv:.2f}, LPIPS: {lpips_value:.4f}", ha='center', va='top', transform=ax.transAxes, fontsize=18)
            else:
                ax.text(0.5, -0.03, f"TV: {tv:.2f}", ha='center', va='top', transform=ax.transAxes, fontsize=18)
