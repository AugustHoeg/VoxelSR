...
if __name__ == '__main__':
    ...
    add = 4
    img_idx_list = [1+add, 10+add, 19+add, 28+add, 46+add, 64+add, 73+add, 55+add, 37, 84]  # For VoDaSuRe_DOWN
    ...
    for i in range(row):
        ...
        red_box_coords = (200, 50)
        red_box_size = 256

        for j in range(col):

            if j == 8:
                large_img_location = (200, 300)

            if j == 9:
                large_img_location = (300, 100)
